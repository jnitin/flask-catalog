...
```bash
(venv) $ pycodestyle *.py application
```

Note that I am not checking the code in the migrations folder, because it is is automatically generated by the Flask-Migrate extension each time we update the database tables.
...
```bash
(venv) $ pylint --rcfile=.pylintrc *.py application
```

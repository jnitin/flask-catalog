...
The tool [pycodestyle](https://pypi.org/project/pycodestyle/) checks that the code conforms to the [PEP 8 -- Style Guide for Python Code](https://www.python.org/dev/peps/pep-0008/). 

To verify that all code in the project follows the checked items of the style guide can be done with a single command:

```bash
(venv) $ pycodestyle *.py */*.py */*/*.py */*/*/*.py
```

You will see that only the code in the migrations folder does not follow the standard, but that is not something we can prevent, because the code is automatically generated by the Flask-Migrate extension each time we update the database tables.
...
The tool [Pylint](https://www.pylint.org/) is complementary to pycodestyle, in that it also checks that the code conforms to the [PEP 8 -- Style Guide for Python Code](https://www.python.org/dev/peps/pep-0008/) , but it finds more issues. I find it easiest to first comply with pycodestyle, and then fix remaining issues found by Pylint. 

To verify that all code in the project follows this style guide can be done with a single command:

```bash
(venv) $ pylint --rcfile=.pylintrc *.py */*.py */*/*.py */*/*/*.py
```
...
```
 ...
 And after creation, it was edited to avoid giving some unavoidable errors related to dynamically generated member attributes of classes and modules, as described [here](https://stackoverflow.com/questions/28193025/pylint-cant-find-sqlalchemy-query-member) and [here](https://www.reddit.com/r/cs50/comments/5zd2n5/pylint_error_message_on_cloud_9_cs50/). 
...
Since I use Wing IDE 6, it was very handy to setup tool panels.

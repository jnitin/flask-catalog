...
from tests_utils import pprint_sequence, pprint_response, ordered
...
class APITestCase(unittest.TestCase):
    def setUp(self):
        ...
        # builtins.AssertionError: View function mapping is overwriting an existing endpoint function: api.api.user_list
        #
        ...
        #self.user_data_list.append(
            #{
                #"data": {
                    #"type": "user",
                    #"attributes": {
                        #"email": '{}'.format(current_app.config['ADMIN_EMAIL']),
    ...
    #def test_0_0_playground_with_populated_database(self):
        ...
        ## break here for playing with populated database, 3 tokens are set for
    ...
    #def test_2_1_verify_relationships(self):
        ...
        ## Verify discovery of weekly summary and verify it's content
        #user_id = response_data['data']['id']
        #url = '/api/v1/users/{}'.format(user_id)
        #response = self.client().get(url, headers=headers)
        #response_data = json.loads(response.data.decode())
        #url = response_data['data']['relationships']['weeks']['links']['related']
        #self.assertEqual(url,'/api/v1/users/3/weeks')
        #response = self.client().get(url, headers=headers)
        #response_data = json.loads(response.data.decode())
    ...
    #def test_3_1_protect_access_to_users_items_weeks(self):
        ...
        ## get this user's data relying on internal permissions.
        #url='/api/v1/users/'  # returns a list!
        #response = self.client().get(url,headers=headers)
        #self.verify_response_is_200_OK(response)
        #response_data = json.loads(response.data.decode())
        ...
        ## get list of user's weeks
        #url='/api/v1/users/3/weeks'
        #response = self.client().get(url,headers=headers)
        #self.verify_response_is_200_OK(response)
        #response_data = json.loads(response.data.decode())

        ## get list of user's weeks relying on internal permissions
        #expected_data = response_data.copy()
        #url='/api/v1/weeks'
        #response = self.client().get(url,headers=headers)
        #self.verify_response_is_200_OK(response)
        ...
        ## verify that this user does not have access to other user's data
        #url='/api/v1/users/4'
        #response = self.client().get(url,headers=headers)
        #self.verify_response_is_403_FORBIDDEN(response)
        ...
        #response = self.client().get(url,headers=headers)
        #self.verify_response_is_403_FORBIDDEN(response)
        ## Also not via a filter object in query string
        #email_other = self.user_data_list[3]['data']['attributes']['email']
        #qs=[{"name":"email","op":"eq","val":"{}".format(email_other)}]
        #url = '/api/v1/users?filter='+json.dumps(qs)
        #response = self.client().get(url, headers=headers)
        #self.verify_response_is_200_OK(response)  # query succeeds
        #response_data = json.loads(response.data.decode())
        #self.assertEqual(response_data['meta']['count'], 0)  # but is empty
    ...
    #def test_3_2_protect_endpoints_if_not_logged_in(self):
        ...
        ##urls_weeks.append('/api/v1/weeks/1/relationships/user')  # WeekRelationship
        ...
        #for url in urls_users:
            ...
            #if url is not '/api/v1/users/':  # registration while not logged in
                #response = self.client().post(url,
                                              #headers=headers,
                                              #data=json.dumps(data))
                #self.verify_response_is_401_or_405(response)
    ...
    #def test_5_0_filtering(self):
        ...
        ## Nasty detail: json.dumps enforces "" for parameters, which is needed
